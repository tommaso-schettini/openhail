#!/bin/bash
# Fleet-location checkpoints: NYC day 1, N in {100,500,1000,2000},
# L in {5,10,20,40}, one training seed, and 500 episodes per run.
# Demand is 20 requests per vehicle and capacity is 0.2 posts per vehicle.
#SBATCH --job-name=ppo-nyc-500ep
...
INSTANCE_FILE="${INSTANCE_FILE:-scripts/instances/periodic_ppo_nyc_day1_4fleets_4location_counts_1seed_500ep_instances.tsv}"
...
EXPERIMENT="nyc_500ep"
RUN_NAME="periodic_ppo_${EXPERIMENT}_${FLEET}_${INFRASTRUCTURE}_s${SEED}_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"

echo "[$(date)] Running fleet-location PPO task ${SLURM_ARRAY_TASK_ID}/16..."
